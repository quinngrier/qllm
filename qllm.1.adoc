...
The current content of the file will be captured and the
{x_atatat_lt}
directive will be converted into an
{x_atatat_ltlt}
directive.
...
The
{x_atatat_ltlt}
directive adds a file to the session with content specified inline.
...
When using the
{x_atatat_ltlt}
directive multiple times in a row, the intermediate terminating
...
{x_dotdot}
components, even if you try to permit it.
