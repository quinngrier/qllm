...
:q_download_url: https://raw.githubusercontent.com/quinngrier/qllm/main/qllm
:q_man_page_url: https://github.com/quinngrier/qllm/blob/main/qllm.1.adoc

Qllm is an LLM CLI tool.
...
. Download the link:{q_download_url}[`qllm`] script and optionally put
it in your `PATH`.

. Set your `VISUAL` environment variable to your text editor.
For example, you can add `export VISUAL=vim` to your `~/.bash_profile`.
...
when opening a file.
For example, you can add `export VISUAL_BOTTOM=+` to your
`~/.bash_profile` if you're using `vim`.
